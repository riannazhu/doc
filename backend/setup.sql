-- Supabase Database Setup for Agentic Document Manager
-- Run this in Supabase SQL Editor: Database → SQL

-- Enable pgvector extension
create extension if not exists vector;

-- Core tables (all snake_case, matching MVP schema)
...
create index if not exists idx_obligation_due_date on obligation(due_date);

-- Verify setup
select 'Setup complete! Tables created:' as message;
select tablename from pg_tables where schemaname = 'public' 
  and tablename in ('document', 'document_page', 'document_embedding', 'extracted_fact', 'obligation');
